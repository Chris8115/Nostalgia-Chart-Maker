...
import { Op3Difficulty, Op3Hand, Op3Note, Op3SongProject, createEmptyProject, defaultDescription, difficulties as projectDifficulties, validateProject } from "./shared/project.js";
...
const aishite: SongConfig = {
  basenameLower: "m_custom0001_aishite",
  basenameUpper: "M_CUSTOM0001_AISHITE",
  title: "Aishite",
  titleKana: "Aishite",
  artist: "Kikuo",
  artistKana: "Kikuo",
  description: "Custom song prototype generated by op3-mapper.",
  bpm: 137,
  audioPath: join(repoRoot, "Aishite Aishite Aishite.mp3"),
  imagePath: join(repoRoot, "song image.jpg"),
  levels: [4, 7, 10, 13]
};
...
function main() {
  ...
  if (command === "install-aishite") {
    installSong(aishite);
    return;
  }

  if (command === "install-aishite-audio") {
    installRealAudio(aishite);
    return;
  }

  if (command === "install-aishite-audio-alias") {
    installAliasedRealAudio(aishite);
    return;
  }

  if (command === "install-aishite-audio-pcm-alias") {
    installAliasedPcmAudio(aishite);
    return;
  }

  if (command === "seed-aishite-project") {
    const outputPath = resolve(readFlag(args, "--output") ?? `${aishite.basenameLower}.op3song.json`);
    writeAishiteProject(outputPath);
    return;
  }
  ...
  if (command === "probe-existing") {
    const basenameArg = readFlag(args, "--basename");
    if (!basenameArg) throw new Error("Missing --basename");
    probeAishiteWithExistingBasename(basenameArg);
    return;
  }

  if (command === "probe-known-assets") {
    const basenameArg = readFlag(args, "--basename");
    if (!basenameArg) throw new Error("Missing --basename");
    installKnownAssetsProbe(basenameArg);
    return;
  }

  if (command === "restore-custom") {
    restoreAishiteCustomBasename();
    installSong(aishite);
    return;
  }
...
function printHelp() {
  console.log(`op3-mapper

Commands:
  install-aishite
  install-aishite-audio
  install-aishite-audio-alias
  install-aishite-audio-pcm-alias
  seed-aishite-project --output m_custom0001_aishite.op3song.json
  ...
  probe-existing --basename M_T0116_qma_milkywaystar
  probe-known-assets --basename M_T0116_qma_milkywaystar
  restore-custom
  remove --basename M_CUSTOM0001_AISHITE
`);
}
...
function probeAishiteWithExistingBasename(existingBasename: string) {
  replaceAishiteBasename(existingBasename);
  console.log(`Aishite entry now points at existing basename ${existingBasename}`);
  console.log("Launch test: if this reaches gameplay, index 406 and the jacket/title assets are probably okay.");
}

function restoreAishiteCustomBasename() {
  replaceAishiteBasename(aishite.basenameUpper);
  console.log(`Aishite entry restored to ${aishite.basenameUpper}`);
}

function installKnownAssetsProbe(existingBasename: string) {
  const existingLower = existingBasename.toLowerCase();
  const sourceFolder = join(musicRoot, existingLower);
  const targetFolder = join(musicRoot, aishite.basenameLower);
  const suffixes = ["00normal", "01hard", "02extreme", "03real"];

  if (!existsSync(sourceFolder)) {
    throw new Error(`Known song folder was not found: ${sourceFolder}`);
  }

  mkdirSync(targetFolder, { recursive: true });
  for (const suffix of suffixes) {
    copyFileSync(
      join(sourceFolder, `${existingLower}_${suffix}.xml`),
      join(targetFolder, `${aishite.basenameLower}_${suffix}.xml`)
    );
  }

  for (const suffix of ["", "_pre"]) {
    for (const extension of [".xsb", ".xwb"]) {
      const sourcePath = join(sourceFolder, `${existingLower}${suffix}${extension}`);
      if (existsSync(sourcePath)) {
        copyFileSync(sourcePath, join(targetFolder, `${aishite.basenameLower}${suffix}${extension}`));
      }
    }
  }

  replaceAishiteBasename(aishite.basenameUpper);
  console.log(`Copied charts/audio from ${existingBasename} into ${aishite.basenameUpper}'s folder.`);
  console.log("Launch test: if this still crashes, the custom basename is likely not accepted by the XACT soundbank.");
}
...
function writeAishiteProject(outputPath: string) {
  const project = createSeedProject();
  writeFileSync(outputPath, `${JSON.stringify(project, null, 2)}\n`, "utf8");
  console.log(`Wrote ${outputPath}`);
}

function createSeedProject(): Op3SongProject {
  const project = createEmptyProject();
  project.id = aishite.basenameUpper;
  project.title = aishite.title;
  project.artist = aishite.artist;
  project.bpm = aishite.bpm;
  project.audioPath = aishite.audioPath;
  project.jacketPath = aishite.imagePath;
  const analysis = analyzeAudio(aishite.audioPath);
  project.durationMs = analysis?.durationMsec ?? readMp3DurationMsec(aishite.audioPath) ?? project.durationMs;
  ...
  for (const difficulty of projectDifficulties) {
    ...
    const count = difficulty === "normal" ? 64 : difficulty === "hard" ? 112 : difficulty === "expert" ? 176 : 248;
    const step = difficulty === "normal" ? beatMs : difficulty === "hard" ? beatMs * 0.75 : difficulty === "expert" ? beatMs * 0.5 : beatMs * 0.375;
    const notes: Op3Note[] = [];
    const activeHolds: Op3Note[] = [];

    for (let i = 0; i < count; i++) {
      const hand: Op3Hand = i % 4 < 2 ? "left" : "right";
      const centerPattern = hand === "left" ? [5, 8, 11, 7, 10] : [17, 20, 23, 18, 21];
      const center = centerPattern[i % centerPattern.length];
      const minKey = clamp(center - Math.floor(width / 2), 0, 27);
      const maxKey = clamp(minKey + width - 1, 0, 27);
      const startMs = Math.round(beatMs * 4 + i * step);
      const isHold = i % (difficulty === "normal" ? 18 : difficulty === "hard" ? 16 : 14) === 0;
      const duration = isHold ? beatMs * (difficulty === "normal" ? 2 : 1.5) : Math.max(100, beatMs * 0.35);

      const candidate: Op3Note = {
        id: `${difficulty}-${i}`,
        startMs,
        ...
        minKey,
        maxKey,
        pitch: projectPitchAt(analysis, startMs, hand, i),
        type: isHold ? "hold" : "tap"
      };

      if (candidate.type === "tap" && activeHolds.some((hold) => notesOverlap(candidate, hold))) {
        continue;
      }

      notes.push(candidate);
      if (candidate.type === "hold") activeHolds.push(candidate);
      while (activeHolds.length > 0 && activeHolds[0].endMs < startMs) activeHolds.shift();
    }
...
function replaceAishiteBasename(nextBasename: string) {
  const original = readFileSync(musicListPath);
  const text = original.toString("latin1");
  const specPattern = /  <music_spec __type='void' index='\d+'>\r?\n(?:(?!  <\/music_spec>).)*?<title\s+__type='str' >Aishite<\/title>(?:(?!  <\/music_spec>).)*?  <\/music_spec>/s;
  const specMatch = text.match(specPattern);

  if (!specMatch || specMatch.index === undefined) {
    throw new Error("Could not find the Aishite music_list entry. Run npm run install:aishite first.");
  }

  const currentSpec = specMatch[0];
  const patchedSpec = currentSpec.replace(
    /<basename\s+__type='str' >[^<]+<\/basename>/,
    `<basename         __type='str' >${nextBasename}</basename>`
  );

  if (patchedSpec === currentSpec) {
    throw new Error("Could not replace basename in Aishite music_list entry.");
  }

  backupOnce(musicListPath);
  const start = Buffer.byteLength(text.slice(0, specMatch.index), "latin1");
  const end = start + Buffer.byteLength(currentSpec, "latin1");
  const patched = Buffer.concat([
    original.subarray(0, start),
    Buffer.from(patchedSpec, "latin1"),
